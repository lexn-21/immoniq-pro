...
import { Trees, Plus, MapPin } from "lucide-react";
...
const LandParcels = () => {
  ...
  const save = async () => {
    ...
    const payload: any = {
      ...
      bodenrichtwert_eur_sqm: form.bodenrichtwert_eur_sqm ? Number(form.bodenrichtwert_eur_sqm) : null,
      parcel_type: form.parcel_type,
      lease_type: form.lease_type,
      lease_holder: form.lease_holder || null,
      lease_annual_eur: form.lease_annual_eur ? Number(form.lease_annual_eur) : null,
      lease_end_date: form.lease_end_date || null,
      notes: form.notes || null,
    };
    const { error } = await supabase.from("land_parcels" as any).insert(payload);
    if (error) return toast.error(error.message);
    toast.success("Grundstück angelegt");
    setOpen(false);
    setForm({
      name: "", gemarkung: "", flur: "", flurstueck: "",
      zip: "", city: "", area_sqm: "", bodenrichtwert_eur_sqm: "",
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2"><Plus className="h-4 w-4" /> Grundstück hinzufügen</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Neues Grundstück</DialogTitle></DialogHeader>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={{ label: "Erstes Grundstück anlegen", onClick: () => setOpen(true) }}
        />
      ) : (
          ...
          {items.map((p) => (
            <Card key={p.id} className="p-4 hover:shadow-md transition">
              <div className="flex items-start justify-between gap-2">
                ...
                <Badge variant="secondary">{PARCEL_TYPES.find(t => t.v === p.parcel_type)?.l ?? p.parcel_type}</Badge>
              </div>
